# renderDXF2SVG_V0.2_colors.py (patched with width-based colormap)
import ezdxf
import svgwrite
import math
import json
...
# === BUILD WIDTH-BASED COLOR MAP ===
widths = [r["width_um"] for r in region_map if r["width_um"] > 0]
...
# === LINES ===
# Skip all LINE elements to disable Voronoi stroke rendering
# (They can be enabled again if desired)
# === HATCHES with WIDTH COLORING ===
throat_counter = 0
for hatch in msp.query("HATCH"):
    try:
        ...
        if layer == "throats":
            width = region_map[throat_counter].get("width_um", 0)
            rgba = scalar_map.to_rgba(width)
